...
class StableZero123Guidance(BaseObject):
    ...
    class Config(BaseObject.Config):
        pretrained_model_name_or_path: str = "models/stable-zero123.ckpt"
    ...
    #读入图片
    @torch.cuda.amp.autocast(enabled=False)
    def prepare_embeddings(self, image_path: str="", image=None) -> None:
        # load cond image for zero123
        assert os.path.exists(image_path) or image is not None, "image is null"
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print(latents.shape)
        decode_latents = self.decode_latents(latents)
        guidance_out = {
            "loss_sds": loss_sds,
            "grad_norm": grad.norm(),
