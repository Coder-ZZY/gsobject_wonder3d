...
@torch.no_grad()
def extract_camera_params(poses):
    c2ws = torch.inverse(poses)
    # Extract translation vector (position of camera in world coordinates)
    Ts = c2ws[:,:3,3].reshape(-1, 3)
    cam_distances = torch.norm(Ts, dim=-1)
    elevations = torch.asin(Ts[:,2]/cam_distances)
    azimuths = torch.atan2(Ts[:,1], Ts[:,0])
    return cam_distances,elevations,azimuths
@torch.no_grad()
def extract_camera_extrs(Rs,Ts):
    bottom = torch.zeros(Ts.shape[0],1,4).cuda()
    bottom[:,:,3] = 1
    return torch.cat([torch.cat([Rs,Ts.reshape(Ts.shape[0],3,1)],dim=-1),bottom],dim = 1)
...
@threestudio.register("test-system")
class Test(BaseLift3DSystem):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        self.zero123_outs: List[torch.Tensor] = []
    ...
    def get_random_view_batch(self, batch: dict[str, Any]) -> dict[str, Any]:
        ...
        camera_distances, azimuths, elevations= extract_camera_params(batch['pose'])
        return {
    ...
    def training_step(self, batch, batch_idx):
        #pose is a w2c maxtrix
        for R, T in batch['random_poses']:
            zero123_batch = batch.copy()
            zero123_batch['random_R'] = R
            zero123_batch['random_T'] = T
            zero123_batch['pose'] = extract_camera_extrs(R,T)
            zero123_batch = self.get_random_view_batch(zero123_batch)
            batch['pose'] = extract_camera_extrs(batch['R'],batch['T'])
            gt_cam_distances, gt_elevations, gt_azimuths = extract_camera_params(batch['pose'])
            for i in range(len(gt_cam_distances)):
                self.guidance.cfg.cond_elevation_deg = gt_elevations[i].cpu().item()*180/np.pi
                self.guidance.cfg.cond_azimuth_deg = gt_azimuths[i].cpu().item()*180/np.pi
                self.guidance.cfg.cond_camera_distance = gt_cam_distances[i].cpu().item()
                self.tensor_to_pil(batch['image'][i]).save(f"test_{i}.png")
                self.guidance.cfg.cond_image_path = f"test_{i}.png"
                self.guidance.prepare_embeddings(image_path=self.guidance.cfg.cond_image_path)
                print(batch['image'].shape)
                guidance_out = self.guidance(rgb=batch['image'].permute(0, 2, 3, 1),elevation=zero123_batch['elevation'],azimuth=zero123_batch['azimuth'],camera_distances=zero123_batch['camera_distances'])
                print("-----------------")
            #test
            #render_sparse_results = self.render_gs(batch, renderbackground=self.background_tensor, need_loss=False)
            # images_sparse = render_sparse_results['images']
            # for idx, timage in enumerate(images_sparse):
            #     self.tensor_to_pil(timage).save(f"{batch['index'][idx]}.png")
            # for idx, timage in enumerate(images):
            #     self.tensor_to_pil(timage).save(f"{batch['random_index'][idx]}.png")
            #-----
        ...
        #         for idx, zero123_out in enumerate(self.zero123_outs):
        #             if zero123_out is None:
        #                 zero123_out = torch.zeros((1, 3, self.novel_image_size, self.novel_image_size), device='cuda')
        #                 self.zero123_outs[idx] = zero123_out
        #         zero123_outs_image = make_grid(torch.cat(self.zero123_outs, dim=0), nrow=5)
        #         save_image(zero123_outs_image, self.get_save_path(f"zero123_out/it{self.true_global_step}.png"))
        #     self.zero123_outs= []
        ...
        #             zero123_batch['random_T'] = T
        #             zero123_batch = self.get_random_view_batch(zero123_batch)
        #             render_results = self.render_gs(zero123_batch, renderbackground=self.background_tensor, need_loss=False)
