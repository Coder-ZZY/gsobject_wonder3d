...
class SingleImageDreamUDFIterableDataset(IterableDataset, SingleImageDreamUDFDataBase, Updateable):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        batch = {
            "rays_o": self.rays_o[idx:idx+1],
            "rays_d": self.rays_d[idx:idx+1],
            # "mvp_mtx": self.mvp_mtx,
            "camera_positions": self.camera_position[idx:idx+1],
            "light_positions": self.light_position[idx:idx+1],
            "elevation": self.elevation_deg,
            "azimuth": self.azimuth_deg[idx:idx+1],
...
class SingleImageDreamUDFDataset(Dataset, SingleImageDreamUDFDataBase):
    def __init__(self, cfg: Any, split: str) -> None:
        super().__init__()
        self.setup(cfg, split)
...
@register("single-image-dreamudf-datamodule")
class SingleImageDreamUDFDataModule(pl.LightningDataModule):
    cfg: SingleImageDreamUDFDataModuleConfig

    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        super().__init__()
        self.cfg = parse_structured(SingleImageDreamUDFDataModuleConfig, cfg)

    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = SingleImageDreamUDFIterableDataset(self.cfg, "train")
        if stage in [None, "fit", "validate"]:
            self.val_dataset = SingleImageDreamUDFDataset(self.cfg, "val")
        if stage in [None, "test", "predict"]:
            self.test_dataset = SingleImageDreamUDFDataset(self.cfg, "test")
