...
class RandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        if self.split == "val":
            ...
            azimuth_deg = torch.cat([torch.linspace(0, 360.0, self.n_views//2+1)[: self.n_views//2] for i in range(2)])
        else:
        ...
        #     azimuth_deg, self.cfg.eval_elevation_deg
        # )
            # elevation_deg: Float[Tensor, "B"] = torch.cat([torch.ones(self.n_views//2)*30*i-15 for i in range(2)])
            elevation_deg: Float[Tensor, "B"] = torch.cat([torch.ones(self.n_views//2)*(75) for i in range(2)])
        else:
            elevation_deg: Float[Tensor, "B"] = torch.full_like(
                azimuth_deg, self.cfg.eval_elevation_deg
            )
