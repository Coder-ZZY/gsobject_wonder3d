...
@threestudio.register("stable-zero123-anchored-guidance")
class Zero123AnchoredGuidance(BaseObject):
    @dataclass
    ...
    def prepare_embeddings(self, image_path: str) -> None:
        ...
        for i in range(6):
            ...
            rgb_256_i: Float[Tensor, "1 3 H W"] = (
            ...
            self.rgb_256.append(rgb_256_i)

        self.c_crossattn = []
        self.c_concat = []
        for i in range(6):
            # print(self.rgb_256[i])
            c_crossattn_i, c_concat_i = self.get_img_embeds(self.rgb_256[i])
            # print(c_crossattn_i,c_crossattn_i.shape)
            # print(c_concat_i,c_concat_i.shape)
            # exit()
            self.c_crossattn.append(c_crossattn_i)
            self.c_concat.append(c_concat_i)
        self.c_crossattn = torch.stack(self.c_crossattn, dim=0)
        self.c_concat = torch.stack(self.c_concat, dim=0)
    ...
    def get_cond(
        ...
        **kwargs,
        ...
        anchored_azimuth = torch.tensor([0,45,90,180,270,315.]).to(azimuth)
        azi_cos = torch.cos((azimuth[:,None]-anchored_azimuth[None,:])/180.*torch.pi).to(azimuth)
        sorted_cos, indices_cos = torch.sort(azi_cos)
        anchored_idx = indices_cos[:, -1].to(azimuth).long()
        cond_azimuth_deg = anchored_azimuth[anchored_idx].to(azimuth)
        T = torch.stack(
            [
                torch.deg2rad(
        ...
        #     ],
        #     dim=-1,
        # )[:, None, :].to(self.device) # b, 1, xxx
        # print(T.shape)
        # exit()
        cond = {}
        # print(self.c_crossattn[anchored_idx].squeeze(1).shape,len(T))
        # print(self.c_concat[anchored_idx].squeeze(1).shape,len(T))
        # print(c_crossattn,c_concat)
        # exit()
        clip_emb = self.model.cc_projection(
            torch.cat(
                [
                    # (self.c_crossattn[anchored_idx] if c_crossattn is None else c_crossattn).repeat(
                    #     len(T), 1, 1
                    # ),
                    (self.c_crossattn[anchored_idx].squeeze(1) if c_crossattn is None else c_crossattn.repeat(len(T), 1, 1)),
                    T,
                ],
                dim=-1,
            )
        ...
        cond["c_concat"] = [
            torch.cat(
                [
                    torch.zeros_like(self.c_concat[anchored_idx].squeeze(1))
                    .to(self.device),
                    # (self.c_concat[anchored_idx] if c_concat is None else c_concat).repeat(
                    #     len(T), 1, 1, 1
                    # ),
                    (self.c_concat[anchored_idx].squeeze(1) if c_concat is None else c_concat.repeat(len(T), 1, 1, 1)),
                ],
                dim=0,
            )
        ]
    ...
    def generate(
        ...
        cond = self.get_cond(
            elevation, azimuth, camera_distances, c_crossattn[0], c_concat[0]
        )
