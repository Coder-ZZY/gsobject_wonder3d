...
@threestudio.register("dreamudf-volume-renderer")
class DreamUDFVolumeRenderer(VolumeRenderer):
    @dataclass
    class Config(VolumeRenderer.Config):
        ...
        num_samples_per_ray_wonder3d: int = 16
    ...
    def configure(
        ...
        if self.cfg.estimator == "occgrid":
            self.estimator = nerfacc.OccGridEstimator(
                roi_aabb=self.bbox.view(-1), resolution=128, levels=1
            )
            if not self.cfg.grid_prune:
                self.estimator.occs.fill_(True)
                self.estimator.binaries.fill_(True)
            self.render_step_size = (
                1.732 * 2 * self.cfg.radius / self.cfg.num_samples_per_ray/10
            )
            self.render_step_size_wonder3d = (
                1.732 * 2 * self.cfg.radius / self.cfg.num_samples_per_ray_wonder3d/10
            )
    ...
    def forward(
        ...
        **kwargs
        ...
        if mode == 'wonder3d':
            return self.render_wonder3d(rays_o, rays_d, light_positions)
        elif mode == 'neudf':
            return render_neudf(rays_o, rays_d, light_positions)
        else:
            pass
        ...
        if len(light_positions.shape) == 2:
            ...
            light_positions_flatten = light_positions.reshape(-1, 3)
    ...
    def render_wonder3d(self,
        rays_o: Float[Tensor, "B H W 3"],
        rays_d: Float[Tensor, "B H W 3"],
        light_positions: Float[Tensor, "B H W 3"],):
        rays_o_flatten: Float[Tensor, "Nr 3"] = rays_o.reshape(-1, 3)
        rays_d_flatten: Float[Tensor, "Nr 3"] = rays_d.reshape(-1, 3)
        light_positions_flatten = light_positions.reshape(-1, 3)
        n_rays = rays_o_flatten.shape[0]
            ...
            if not self.cfg.grid_prune:
                with torch.no_grad():
                    ray_indices, t_starts_, t_ends_ = self.estimator.sampling(
                        ...
                        near_plane=self.cfg.near_plane,
                        far_plane=self.cfg.far_plane,
                        render_step_size=self.render_step_size_wonder3d,
                        alpha_thre=0.0,
                        stratified=self.randomized,
                        cone_angle=0.0,
                        early_stop_eps=0,
        ...
        out = {
            ...
            "comp_rgb_fg": comp_rgb_fg.view(-1, 3),
            "comp_rgb_bg": comp_rgb_bg.view(-1, 3),
            "opacity": opacity.view(-1, 1),
            "depth": depth.view(-1, 1),
            "z_variance": z_variance.view(-1, 1),
        }
        ...
        if self.training:
            ...
            if "normal" in geo_out:
                ...
                out.update(
                    {
                        "comp_normal": comp_normal.view(-1, 3),
                    }
                )
