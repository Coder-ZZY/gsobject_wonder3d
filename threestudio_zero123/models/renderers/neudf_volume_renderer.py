...
@threestudio.register("neudf-volume-renderer")
class NeUDFVolumeRenderer(VolumeRenderer):
    @dataclass
    class Config(VolumeRenderer.Config):
        ...
        cos_anneal_end_steps: int = 1000
        udf_anneal_end_steps: int = 10000
    ...
    def get_alpha(self, udf, normal, dirs, dists):
        inv_std = self.variance(udf)
        ...
        estimated_next_udf = udf + iter_cos * dists * 0.5
        estimated_prev_udf = udf - iter_cos * dists * 0.5

        prev_cdf = (estimated_prev_udf * inv_std)/(1 + estimated_prev_udf * inv_std)
        next_cdf = (estimated_next_udf * inv_std)/(1 + estimated_next_udf * inv_std)
    ...
    def forward(
        ...
        **kwargs
        ...
        #     if self.training:
        #         udf = self.geometry.forward_udf(positions)[..., 0]
        #     else:
        #         udf = chunk_batch(
        #             self.geometry.forward_udf,
        ...
        #     estimated_next_udf = udf - self.render_step_size * 0.5
        #     estimated_prev_udf = udf + self.render_step_size * 0.5
        #     prev_cdf = (estimated_prev_udf * inv_std)/(1 + estimated_prev_udf * inv_std)
        #     next_cdf = (estimated_next_udf * inv_std)/(1 + estimated_next_udf * inv_std)
        #     p = prev_cdf - next_cdf
        #     c = prev_cdf
        ...
        alpha: Float[Tensor, "Nr 1"] = self.get_alpha(
            geo_out["udf"], geo_out["normal"], t_dirs, t_intervals
        )

        weights_neudf: Float[Tensor, "Nr 1"]
        weights_neudf_, _ = nerfacc.render_weight_from_alpha(
            alpha[..., 0],
            ray_indices=ray_indices,
            n_rays=n_rays,
        )
        weights_neudf = weights_neudf_[..., None]

        weights_density_, _, _ = nerfacc.render_weight_from_density(
            t_starts[..., 0],
            t_ends[..., 0],
            geo_out["density"][..., 0],
            ray_indices=ray_indices,
        ...
        weights_density = weights_density_[..., None]

        weights = weights_neudf * self.beta + weights_density * (1. - self.beta)
        ...
        if self.training:
            ...
            out.update({'points': geo_out['points']})
            out.update({'udf_grad': geo_out['udf_grad']})
    ...
    def update_step(
        ...
        if self.cfg.grid_prune:
            ...
            def occ_eval_fn(x):
                udf = self.geometry.forward_udf(x)
                inv_std = self.variance(udf)

                estimated_next_udf = udf - self.render_step_size * 0.5
                estimated_prev_udf = udf + self.render_step_size * 0.5
                # prev_cdf = torch.sigmoid(estimated_prev_udf * inv_std)
                # next_cdf = torch.sigmoid(estimated_next_udf * inv_std)
                prev_cdf = (estimated_prev_udf * inv_std)/(1 + estimated_prev_udf * inv_std)
                next_cdf = (estimated_next_udf * inv_std)/(1 + estimated_next_udf * inv_std)
                # p = prev_cdf - next_cdf
                # c = prev_cdf
        ...
        self.beta = (
            1.0
            if self.cfg.udf_anneal_end_steps == 0
            else min(1.0, (global_step / self.cfg.udf_anneal_end_steps)**2)
        )
