...
class Zero123(BaseLift3DSystem):
    ...
    def training_substep(self, batch, batch_idx, guidance: str):
        ...
        if guidance == "ref":
            ...
            set_loss("rgb", ranking_loss(F.mse_loss(gt_rgb, out["comp_rgb"], reduction='none').reshape(-1),penalize_ratio=0.8,type='mean'))

            # mask loss
            # set_loss("mask", F.mse_loss(gt_mask.float(), out["opacity"]))
            set_loss("rgb", ranking_loss(F.mse_loss(gt_mask.float(), out["opacity"], reduction='none').reshape(-1),penalize_ratio=0.8,type='mean'))
